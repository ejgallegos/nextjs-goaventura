...
8. The main home hero starts with one informational institutional slide saying `Primero elegí dónde quedarte. Después, viví La Rioja.`; it then includes accommodation slides with real interior photos and published experiences only (no promotions). Accommodation/experience slides have their contextual internal CTA. Auto-advance is 6.5 seconds, stops for reduced motion and pauses on hover/focus or explicit Pausar/Reanudar control with `aria-pressed`; keyboard arrows remain available.
...
- AF-09 scope update: the earlier standalone AF-08 attempt was reverted. The approved slider replaces the main static hero. After the later authorized carousel update, its first slide is informational and says `Primero elegí dónde quedarte. Después, viví La Rioja.`; subsequent slides use property interiors and published experiences only, with promotions excluded. Accommodation/experience slides keep one contextual internal Link CTA; the institutional slide has none. Auto-advance is permitted at 6.5 seconds but is disabled for reduced-motion preference and pauses on hover/focus or user control. Preserve keyboard navigation, logo palette, and the remaining home content.
...
- AF-09 implementation: `src/components/home-hero-slider.tsx` provides a full-width slider with an informational first slide, accommodation/experience slides, contextual internal CTA only on product slides, and discreet previous/next controls. It auto-advances every 6.5 seconds unless reduced motion is preferred, and pauses on hover/focus and via an explicit `aria-pressed` Pausar/Reanudar button; arrow keys remain available. Focus, 44px controls, brand colors, `sizes="100vw"`, and first-image priority remain. `src/app/page.tsx` uses distinct existing interior photos for each accommodation and published products only; offers are capped so accommodation slides remain at least half. Promotions remain available in the separate `Completá tu estadía` section but are excluded from the slider. Existing stay slides provide useful content while offers load or fail.
...
- AF-09 informational/autoplay/interior update: the first slide now states `Primero elegí dónde quedarte. Después, viví La Rioja.` and explains Go Aventura's lodging-first offer; it has no CTA. Promotions were removed from all hero slide data/types/CTAs. Published experience slides only follow the informational and lodging slides. Inspected the image arrays and local image files; selected interiors are Loft Centro `loft-centro-2.jpg` (bedroom), Casa Altos del Talampaya `casa-10.jpg` (living room), Casa II `casa-ii-12.jpg` (bedroom), Casa III `casa-iii-14.jpg` (bedroom); the institutional image is the existing Casa I interior `casa-altos-i-interior-patio.jpg`. The carousel advances every 6.5 seconds only when reduced motion is not preferred, pauses while hovered/focused, and provides a Pausar/Reanudar control with `aria-pressed`; keyboard navigation remains available.
- AF-09 latest checks: `npm run lint -- --file src/components/home-hero-slider.tsx --file src/app/page.tsx` passed with no warnings/errors; `npm run typecheck` passed; `npm run build` passed and generated all 50 static pages; `git diff --check` passed. Build emitted the known optional Jaeger exporter/Handlebars warnings. Strict TDD RED remains unavailable because no general runner is configured; no RED evidence was invented.
