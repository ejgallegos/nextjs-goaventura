...
- [x] AF-06 — Separate shared travel-card media from its content/actions so image clipping cannot overlap buttons; preserve visible focus and responsive layout.
...
- AF-06 implementation: `src/components/product-card.tsx` now uses a dedicated media frame, independent content body, and separate action grid; the home uses this shared card for its complementary trip/promotion offers.
- AF-06 checks: `npm run lint -- --file src/components/product-card.tsx` passed with no warnings or errors; `npm run typecheck` passed; `git diff --check` passed.
