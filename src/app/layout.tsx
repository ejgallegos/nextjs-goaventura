
import type { Metadata } from 'next';
import './globals.css';
import { Toaster } from "@/components/ui/toaster";
import Header from '@/components/layout/header';
import Footer from '@/components/layout/footer';
import { FloatingWhatsApp } from '@/components/floating-whatsapp';
import { ThemeProvider } from '@/components/theme-provider';
import { RecaptchaProvider } from './recaptcha-provider';
import Script from 'next/script';
import { Roboto, Montserrat } from 'next/font/google';

const roboto = Roboto({
  subsets: ['latin'],
  weight: ['300', '400', '500', '700'],
  variable: '--font-roboto',
  display: 'swap',
});

const montserrat = Montserrat({
  subsets: ['latin'],
  weight: ['600', '700', '800', '900'],
  variable: '--font-montserrat',
  display: 'swap',
});

const siteUrl = process.env.NEXT_PUBLIC_SITE_URL || 'https://goaventura.com.ar';

export const metadata: Metadata = {
  metadataBase: new URL(siteUrl),
  title: {
    default: "Go Aventura — Descubrí La Rioja",
    template: "%s — Go Aventura",
  },
  description:
    "Excursiones en 4x4, transfers y alojamientos premium en Villa Unión del Talampaya. Viví La Rioja con los mejores guías locales.",
  openGraph: {
    title: {
      default: "Go Aventura — Descubrí La Rioja",
      template: "%s — Go Aventura",
    },
    description:
      "Excursiones en 4x4, transfers y alojamientos premium en Villa Unión del Talampaya.",
    url: siteUrl,
    siteName: "Go Aventura",
    images: [
      {
        url: "/logo-goaventura.png",
        width: 1200,
        height: 630,
        alt: "Go Aventura — Agencia de Viajes en Villa Unión del Talampaya",
      },
    ],
    locale: "es_AR",
    type: "website",
  },
  robots: {
    index: true,
    follow: true,
    googleBot: {
      index: true,
      follow: true,
      "max-video-preview": -1,
      "max-image-preview": "large",
      "max-snippet": -1,
    },
  },
  twitter: {
    card: "summary_large_image",
    title: "Go Aventura — Descubrí La Rioja",
    description:
      "Excursiones, transfers y alojamientos en Villa Unión del Talampaya.",
    images: ["/logo-goaventura.png"],
  },
};

export default function RootLayout({
  children,
}: Readonly<{
  children: React.ReactNode;
}>) {
  const jsonLd = {
    "@context": "https://schema.org",
    "@type": "TravelAgency",
    name: "Go Aventura",
    description:
      "Excursiones en 4x4, transfers y alojamientos premium en Villa Unión del Talampaya, La Rioja",
    url: siteUrl,
    logo: `${siteUrl}/logo.png`,
    image: `${siteUrl}/logo-goaventura.png`,
    telephone: "+5493825575566",
    email: "info@goaventura.com.ar",
    address: {
      "@type": "PostalAddress",
      streetAddress: "San Martín S/N",
      addressLocality: "Villa Unión",
      addressRegion: "La Rioja",
      postalCode: "F5350",
      addressCountry: "AR",
    },
    openingHours: "Mo,Tu,We,Th,Fr,Sa,Su 24:00",
    sameAs: ["https://www.instagram.com/goaventura.ok"],
    priceRange: "$$",
    makesOffer: [
      {
        "@type": "Offer",
        "name": "Excursiones en 4x4",
        "description": "Explorá Laguna Brava, Talampaya, el Cañón del Triásico y más con guías expertos.",
        "url": `${siteUrl}/viajes?filter=Excursion`
      },
      {
        "@type": "Offer",
        "name": "Transfers Privados",
        "description": "Traslados seguros y confiables desde y hacia cualquier punto de interés.",
        "url": `${siteUrl}/viajes?filter=Transfer`
      },
      {
        "@type": "Offer",
        "name": "Alojamientos",
        "description": "Lofts, casas y departamentos con las mejores vistas de Villa Unión.",
        "url": `${siteUrl}/alojamientos`
      }
    ],
  };

  const gaMeasurementId = "G-XXEKVSC46X";

  return (
    <html lang="es" suppressHydrationWarning className={`${roboto.variable} ${montserrat.variable}`}>
      <head>
        <link rel="preconnect" href="https://www.googletagmanager.com" />
        <link rel="dns-prefetch" href="https://www.googletagmanager.com" />
        <script
          type="application/ld+json"
          dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd) }}
        />
        <Script async src={`https://www.googletagmanager.com/gtag/js?id=${gaMeasurementId}`}></Script>
        <Script id="google-analytics">
          {`
            window.dataLayer = window.dataLayer || [];
            function gtag(){dataLayer.push(arguments);}
            gtag('js', new Date());
            gtag('config', '${gaMeasurementId}');
          `}
        </Script>
      </head>
      <body className="font-body antialiased flex flex-col min-h-screen">
        <ThemeProvider
            attribute="class"
            defaultTheme="light"
            enableSystem
            disableTransitionOnChange
          >
          <RecaptchaProvider>
            <Header />
            <main className="flex-grow pt-20 lg:pt-24">
              {children}
            </main>
            <Footer />
            <FloatingWhatsApp />
            <Toaster />
          </RecaptchaProvider>
        </ThemeProvider>
      </body>
    </html>
  );
}
